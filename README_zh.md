...
### 🆕 IDM-GridCore - 新一代分布式计算框架

我们为此数据库专门开发了全新的分布式计算框架：

👉 **[IDM-GridCore](https://github.com/Wolido/idm-gridcore)** 👈

**IDM-GridCore** 是一个众筹式分布式并行计算框架。成百上千的计算节点可以像众筹一样共同参与任务，轻松实现万亿级数据并行计算。轻量级、异构支持、边缘设备友好。

### 旧版计算过程

旧版数据计算过程的代码放置于calc_descriptors路径下。计算过程由一个脚本发送计算任务到Redis队列，由多个进程读取队列中的任务并计算。
